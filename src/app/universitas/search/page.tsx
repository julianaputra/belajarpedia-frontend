...
import { fetchUniversitasSearch } from "@/lib/api/facilities";
...
export async function generateMetadata({ searchParams }: Props): Promise<Metadata> {
  ...
  return searchMetadata({
    ...
    query,
    page,
    path: `/universitas/search?q=${encodeURIComponent(query)}`,
  });
}

export default async function UniversitasSearchPage({ searchParams }: Props) {
  const { q, page: pageRaw } = await searchParams;
  const query = (q ?? "").trim();
  const page = pageOf(pageRaw);

  const results = query
    ? await safe(() => fetchUniversitasSearch(query, page))
